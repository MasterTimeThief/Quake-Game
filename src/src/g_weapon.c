...
static void Grenade_Explode (edict_t *ent)
{
	...
	{	
		...
		if (ent->owner->client->resp.levelMutant == 1) // Gunner        ///mtl23 again to remove these errors all you have to do is define these functions at the top of the file, like they are defined in m_gunner.c etc. 
			SP_monster_gunner(mutant); 
		if (ent->owner->client->resp.levelMutant == 2) // Mutant       ///mtl23 see note above
			SP_monster_mutant(mutant);
		if (ent->owner->client->resp.levelMutant == 3) // Gladiator		///mtl23 see note above
